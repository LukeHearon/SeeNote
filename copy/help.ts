...
export const help = {
  ...
  modes: {
    ...
    get li_sweep() { return getOverride('help.modes.li_sweep') ?? "**While listening:** hold `Shift` during playback to mark out what you're hearing. The press drops the start, the playhead carries the end — a dotted line runs down the middle of the span as it grows, as it does for any selection — and letting go makes the selection and stops playback, unless you're also holding `Alt`/`Option`, which keeps it rolling. A quick tap of `Shift` does nothing, so the key stays safe to lean on, and the gesture is only offered when nothing is selected yet."; },
