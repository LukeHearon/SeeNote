...
function useDemoState(): { snapshot: LiveSnapshot; set: (patch: Partial<LiveSnapshot>) => void; store: CurrentTimeStore } {
  ...
  const [snapshot, setSnapshot] = useState<LiveSnapshot>({
    ...
    selectedTimeDisplayUnit: 'seconds',
    // The demo track stands in for one whose filename carries a timestamp, so
    // the Date unit is demonstrable with no project open.
    trackStartMs: DEMO_TRACK_START.getTime(),
    dateTimeFormat: 'iso',
      ...
      case 'time':
        return (
          <TimeReadout
            ...
            selectedUnit={s.selectedTimeDisplayUnit}
            trackStartDate={s.trackStartMs === null ? null : new Date(s.trackStartMs)}
            dateTimeFormat={s.dateTimeFormat}
            onSeek={(t, scroll) => act(() => client.call('seek', t, scroll), () => demo.store.set(t))}
            onUnitChange={(u: TimeDisplayUnit) => act(() => client.call('setTimeDisplayUnit', u), () => demo.set({ timeDisplayUnit: u, selectedTimeDisplayUnit: u }))}
          />
        );
      case 'selection':
        return (
