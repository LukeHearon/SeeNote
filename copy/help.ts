...
export const help = {
  ...
  bulk: {
    ...
    get p_find() { return getOverride('help.bulk.p_find') ?? "Every match is a chip in the results list, in its tool's color, labelled with its start time, and — in a whole-project search — grouped under the recording it belongs to. Click one to open that file, scroll the spectrogram to it, and select the annotation — with an active selection bound around it, ready to edit, resize, or delete. The chevrons at the top right step to the previous and next match (or `Shift+Enter` / `Enter` from the search field), wrapping round at the ends, and the readout beside them says where in the list you are. The list doesn't go anywhere as you walk it: the current match stays highlighted and scrolls into view, across track switches and through any edits you make along the way."; },
