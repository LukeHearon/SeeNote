...
export const help = {
  ...
  filePanel: {
    ...
    get p_pinned() { return getOverride('help.filePanel.p_pinned') ?? "Scrolling through a deep folder's files pins the folders you're inside to the top of the list, one row per level, with a line under the stack showing that there are files hidden above it. The panel keeps at least three tracks visible, so levels that don't fit stop pinning — make the panel taller and they come back."; },
