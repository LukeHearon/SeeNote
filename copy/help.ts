...
export const helpPanel = {
  ...
  guide: {
    ...
    buzzdetect: {
      ...
      get p4() { return getOverride('helpPanel.guide.buzzdetect.p4') ?? "Hovering highlights the unit under the cursor — a single frame while frames are individually visible, otherwise the whole bin — and clicking and dragging work on that same unit. **Click** one to move the playhead to its start and select it (highlighting that audio on the spectrogram); **drag** across the panel to extend the selection, which always snaps to whole units. **Shift+click** extends the current selection to also cover the clicked unit. One exception: where units are drawn only a pixel or two wide — zoomed far out, whether that's tiny frames or bins — a click only moves the playhead, since the unit under the cursor is too small to have been aimed at. Selecting there takes a drag (move a short distance, or hold the button down for a moment and then move). Drag the panel's top edge to resize it. The readout in the top-left corner is headed **Time** (the span shown) and **Activations** for a single frame or **Mean Activations** when it's summarizing a bin, followed in each case by the values underneath."; },
      get p6() { return getOverride('helpPanel.guide.buzzdetect.p6') ?? "**Subsetting to detections.** Tick a neuron's **Sub** box in the sliders popover and the track collapses to only the time that neuron fired: the stretches in between are removed from the time axis entirely, so the detections sit end to end and playback runs straight through them with no gap. Tick several neurons to keep time where **any** of them fired. Toggle the subset off and on with the [scissors icon](buzzdetect-subset-toggle) in the toolbar or `Shift+S`; your neuron picks survive the toggle. Contiguous detections merge into one segment, so a run of frames reads — and selects — as one stretch of audio. The seam between two segments is marked with a dashed line on the spectrogram and a solid flagged line on this panel, so you can see where one detection ends and the next begins."; },
      get p7() { return getOverride('helpPanel.guide.buzzdetect.p7') ?? "While subset, the panel plots bare points rather than a line: neighbouring points can be far apart in the file, and a line between them would draw a trend across time that has been removed. In **detection rate** mode, set a **Min detection rate** to keep whole bins rather than single frames — a bin survives when at least that fraction of its frames fired. The rate is measured over the **pinned** bin width, not the auto one (which follows the zoom, and would silently redefine the subset every time you moved the view); pin a bin width first."; },
      get p8() { return getOverride('helpPanel.guide.buzzdetect.p8') ?? "Times, the ruler, and the toolbar readouts are all on the subset's own timeline, the way they would be for a file of just the extracted audio. Annotations are still stored against real positions in the source file, so what you export is unaffected. A selection or annotation can't cross a cut — drags stop at the segment edge, and `{mod}+A` selects the segment holding the playhead rather than the whole track. Video is switched off while subset: the picture can't follow a timeline that jumps between distant parts of the file, so video tracks play their audio through the same path as audio tracks."; },
      get p5() { return getOverride('helpPanel.guide.buzzdetect.p5') ?? "Zoom out far enough and individual frames stop being distinguishable: the dots drop away, several frames are grouped into each plotted point, and hovering, clicking and dragging switch from single frames to whole bins — the highlight, readout and selection all cover the bin the frames were grouped into. While frames _are_ individually visible, a detection rate is a per-frame yes/no rather than a percentage — the Y axis is pinned to a single **Detection** label at the top (your Y-range override doesn't apply), and the readout reads **Detection** / **No Detection** per neuron."; },
    },
  },
  annotations: {
    tools: {
