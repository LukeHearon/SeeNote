#!/usr/bin/env bash
#
# Create a self-signed code signing certificate for SeeNote's macOS builds.
#
# Why: an unsigned .app has no stable identity, so macOS TCC (Documents /
# Downloads / Desktop access) and Keychain ACLs have nothing durable to attach
# an approval to and re-prompt users after rebuilds and sometimes after plain
# relaunches. A self-signed certificate gives the bundle a fixed designated
# requirement, so approvals stick across launches and across version updates.
#
...
#
# Produces ~/.seenote-signing/ (gitignored territory, outside the repo) and
# prints the two values to store as GitHub Actions secrets.

set -euo pipefail

CN="${1:-SeeNote Self-Signed}"
OUT="$HOME/.seenote-signing"
