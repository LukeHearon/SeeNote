...
export const helpPanel = {
  ...
  guide: {
    ...
    buzzdetect: {
      ...
      get p4() { return getOverride('helpPanel.guide.buzzdetect.p4') ?? "**Click** a frame to move the playhead to its start; a quick click only seeks. To select (and highlight) audio on the spectrogram, **drag** across the panel — either move a short distance, or hold the button down for a moment and then move. **Shift+click** extends the current selection to also cover the clicked frame. Drag the panel's top edge to resize it."; },
      get p5() { return getOverride('helpPanel.guide.buzzdetect.p5') ?? "Zoom out far enough and individual frames stop being distinguishable: the dots drop away, several frames are grouped into each plotted point, the hover readout covers that whole group, and dragging selects a free time range instead of snapping to frame edges. While frames _are_ individually visible, a detection rate is a per-frame yes/no rather than a percentage — the Y axis is pinned to a single **Detection** label at the top (your Y-range override doesn't apply), and the readout reads **Detection** / **No Detection** per neuron."; },
    },
  },
  annotations: {
    tools: {
