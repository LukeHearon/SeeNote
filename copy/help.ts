...
export const help = {
  ...
  layout: {
    ...
    get p2() { return getOverride('help.layout.p2') ?? "The [header](toolbar) across the top holds the back button, the project name and its [settings gear](project-settings-btn@project-settings), the sync controls when [syncing](@sync) is configured, and the debug and help buttons."; },
  ...
  video: {
    ...
    get p_pane2() { return getOverride('help.video.p_pane2') ?? "By default the pane auto-collapses for audio-only tracks and re-opens for video tracks (a video file keeps its pane even if its frames can't be decoded). Toggle this off under **Video Pane** in the [Preferences tab](project-settings-btn@project-settings#prefs-tab) of Project Settings to control the pane manually."; },
  ...
  subset: {
    get p1() { return getOverride('help.subset.p1') ?? "A subset collapses the track to only the time your chosen neurons fired. The stretches in between are removed from the time axis outright rather than dimmed, so the detections sit end to end and playback runs straight through them — as if you had extracted them into a new file. Toggle it with the [scissors button](buzzdetect-subset-toggle) in the toolbar or `Shift+S`."; },

    get h_picking() { return getOverride('help.subset.h_picking') ?? "Choosing what to keep"; },
    get p_picking() { return getOverride('help.subset.p_picking') ?? "Open the [buzzdetect panel](buzzdetect-toggle@buzzdetect)'s **sliders** popover and tick a neuron's **Sub** box. Tick several to keep time where **any** of them fired. The scissors button only appears once something is ticked — with no neuron chosen there would be nothing to subset by. Your picks survive turning the subset off and on, and are saved with the project."; },
    get p_bins() { return getOverride('help.subset.p_bins') ?? "A subset keeps whole **bins**, never single frames: a bin is kept at its own nominal edges or not at all. In **activation** mode a bin is kept when any picked neuron's **mean** activation across it clears that neuron's threshold; in **detection rate** mode, set a **Min detection rate** and a bin is kept when at least that fraction of its frames individually clear the threshold. Either way the bin is the **pinned** bin width — the auto width follows the zoom, and subsetting by it would silently redefine the subset every time you moved the view — so pin a width first. With none pinned, the file's own frame length is used, where a bin is just one frame."; },
    get note_try() { return getOverride('help.subset.note_try') ?? "The example panel below subsets for real. Tick a **Sub** box in its sliders popover and watch the axis collapse to that neuron's detections; raise the neuron's threshold to keep less."; },

    get h_reading() { return getOverride('help.subset.h_reading') ?? "Reading a subset track"; },
    get p_segments() { return getOverride('help.subset.p_segments') ?? "Contiguous detections merge into one **segment**, so a run of frames reads — and selects — as a single stretch of audio. The seam where one segment is spliced to the next is drawn as a dashed gold line, on the spectrogram and the buzzdetect panel alike, so a cut never passes for continuous audio. The panel also stops connecting its points across a seam: neighbouring segments can be far apart in the file, and a line between them would draw a trend across time that has been removed."; },
  ...
  projectSettings: {
    ...
    get p_settings() { return getOverride('help.projectSettings.p_settings') ?? "The project's **name** (and its color gradient), its **Media** and **Annotations** directories, and **Output Decimal Places** — how many decimals timestamps are written with in annotation files. **Filename Timestamp** declares the timestamp pattern your filenames carry (`YYYY`/`YY`, `MM`, `DD`, `HH`, `mm`, `ss`, everything else literal) — that's what unlocks wall-clock [Date mode](@time-display). Under **Advanced** sit the **buzzdetect** directory and frame length; under **Sync**, the repository URL, access token, and your annotator name."; },
    get h_prefs() { return getOverride('help.projectSettings.h_prefs') ?? "Preferences tab"; },
    get p_prefs() { return getOverride('help.projectSettings.p_prefs') ?? "**Auto-collapse video panel** folds the video pane away on audio-only tracks and re-opens it on video ones; turn it off to control the pane by hand. **Automatically pull remote changes** governs background pulls when syncing is set up — see [Sync (GitHub)](@sync). **Datetime Format** picks how wall-clock times are written — `ISO 8601` (`2026-07-31 16:56:04`) or `Friendly` (`July 31, 4:56:04p`) — wherever [Date mode](@time-display) is showing them."; },
