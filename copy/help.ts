...
export const help = {
  ...
  buzzdetect: {
    get p1() { return getOverride('help.buzzdetect.p1') ?? "Set a **buzzdetect directory** under **Advanced** when creating or editing a project to plot per-frame neuron activations below the spectrogram, located per track by ident (`{ident}_buzzdetect.csv`). While a run is still in progress, a `{ident}_buzzpart.csv` is read instead, so the panel fills in as detection proceeds. Toggle the panel with the [activity icon](buzzdetect-toggle) in the toolbar."; },
