...
export const helpPanel = {
  ...
  guide: {
    ...
    timeDisplay: {
      ...
      get p3() { return getOverride('helpPanel.guide.timeDisplay.p3') ?? "**Date** shows real wall-clock times instead of elapsed ones — in the running time, the selection fields, and along the spectrogram's time axis. It needs to know when the recording started, which it reads from the filename: set a **Filename Timestamp** pattern in project settings (e.g. `YYMMDD_HHMM` for `260731_0456.mp3`) and the toggle lights up on every track whose name matches. On a track whose name doesn't match, times stay in whichever of Seconds or HMS you last used. Typing into a time field accepts a clock time (`23:00`) or a full datetime (`2026-07-31 04:56:00`) while Date is active."; },
    },
    autoSave: {
