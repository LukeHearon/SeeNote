...
export const helpPanel = {
  ...
  guide: {
    ...
    buzzdetect: {
      ...
      get p2() { return getOverride('helpPanel.guide.buzzdetect.p2') ?? "Each neuron is one colored line; its dots are filled where the value meets that neuron's threshold and open below it. Open the panel's **sliders** popover to set per-neuron thresholds, show/hide neurons, click a neuron's color swatch to remap its color, and edit the Y-axis range (auto-calculated from the file on load; type over either bound to pin it, or click the reset arrow that appears to recalculate). **Click a frame** to select (and highlight) its audio on the spectrogram; drag across frames to extend the selection. **Shift+click** a frame to extend the current selection to also cover it. Drag the panel's top edge to resize it."; },
