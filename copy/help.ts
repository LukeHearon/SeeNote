...
export const helpPanel = {
  ...
  guide: {
    ...
    buzzdetect: {
      ...
      get p3() { return getOverride('helpPanel.guide.buzzdetect.p3') ?? "Bin width is normally auto-detected per file from the CSV. Under **Advanced → buzzdetect** in project settings, check **Override** to set an explicit **frame length** for every file instead — useful when a CSV has too few rows to auto-detect from. Frames keep their start times from the CSV, so an override shorter than the native frame length leaves gaps between frames — the plot shows each frame's true extent, and clicking in a gap selects nothing."; },
    },
  },
  annotations: {
    tools: {
