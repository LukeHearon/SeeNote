...
export const help = {
  ...
  tools: {
    ...
    get p_manage() { return getOverride('help.tools.p_manage') ?? "Open [Annotation Tool Settings](tool-palette) (gear icon) to manage tools: drag tools between hotkey slots and the Unassigned bin, click a tool's gear to edit its label and color, or hover a tool and click the trash icon to delete it (deletes are undoable). Type into an empty hotkey slot's **Add tool** field to assign an existing tool by name (pick it from the dropdown) or, if the name is new, open the editor to create a tool on that key; in the Unassigned bin, click **New tool** to create one directly. Annotations are linked to a tool by their label, so creating or renaming a tool to match a label instantly adopts every annotation carrying it — across all tracks. Tool configuration is saved per project: each tool is a folder under the project's **.seenote/annotation-tools/** directory, which can also hold example audio clips for that label."; },
