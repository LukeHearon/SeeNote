...
export const help = {
  ...
  modes: {
    ...
    get li_sweep() { return getOverride('help.modes.li_sweep') ?? "**While listening:** hold `Shift` during playback to mark out what you're hearing. The press drops the start, the playhead carries the end — a dotted line marks the middle of the span as it grows — and letting go makes the selection. A quick tap of `Shift` does nothing, so the key stays safe to lean on, and the gesture is only offered when nothing is selected yet."; },
