...
export const help = {
  ...
  buzzdetect: {
    ...
    get p_reading() { return getOverride('help.buzzdetect.p_reading') ?? "Each neuron is one colored line; its dots are filled where the value meets that neuron's threshold and open below it. Which neurons are plotted, and how, is the **Neurons** palette in the left sidebar.\n\nOnly plotted neurons get a row there — a model carries more neurons than any one investigation uses. **Add neuron** at the foot of the list opens a searchable picker of the rest; **Remove** (in a neuron's settings, or its right-click menu) takes a row away again without forgetting its color or thresholds, and the **All** / **None** links by the header do the same to every neuron at once. Each row shows the neuron's color, its name, and its detection threshold, editable in place. Clicking the row toggles whether the track is [subset](@subset) to that neuron.\n\nA neuron's **color dot** — or the gear beside its threshold — opens its settings: color, detection threshold, the separate subset threshold, pin, and remove.\n\nThe **sliders** button in the palette header holds what applies to the whole graph: the **activation** or **detection rate** series (the percentage of frames in each bin clearing the threshold), the bin width used to group frames into each plotted point (auto-calculated to keep the line readable at any zoom; type over it to pin a value — never below the file's frame hop — or click the reset arrow to recalculate), and the Y-axis range (same auto/override/reset pattern). The series choice and a pinned bin width are saved with the project and stay put as you switch tracks; the Y range starts fresh on each track, and switching series resets both."; },
  ...
  subset: {
    ...
    get p_picking() { return getOverride('help.subset.p_picking') ?? "Click a neuron's row in the **Neurons** palette to subset the track to its detections; the row lights up while it's cutting. Pick several to keep time where **any** of them fired. Your picks survive turning the subset off and on, and are saved with the project.\n\nUnder **Subset to detections** in the palette's **sliders** settings, a readout says what the cut came to: how much audio it kept out of the file's length, over how many separate regions, holding how many activation frames. That's the number to steer by while dragging a threshold — a cut that leaves four seconds of the night is usually a threshold set too high."; },
