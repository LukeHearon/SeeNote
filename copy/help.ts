...
export const help = {
  ...
  spectrogram: {
    ...
    get p_freq() { return getOverride('help.spectrogram.p_freq') ?? "The two number boxes on the frequency axis — one at the top, one at the bottom — clip the vertical axis to a band of interest, giving the frequencies you care about the full height of the panel. Type a value and press `Enter` or click away to apply — plain Hz or the same `k` shorthand the axis uses (`8000` or `8k`) — and they clamp to 0 and the file's Nyquist limit. A reset arrow on each box, once it's moved off its limit, snaps it back. Useful for narrowing onto a species' band, or for cutting off the empty top half of a recording sampled far above its content. This is a re-coloring of what's already computed, so it applies instantly."; },
  ...
  buzzdetect: {
    get p1() { return getOverride('help.buzzdetect.p1') ?? "Set a **buzzdetect directory** under **Advanced** when creating or editing a project to plot per-frame neuron activations below the spectrogram, located per track by ident (`{ident}_buzzdetect.csv`). While a run is still in progress, a `{ident}_buzzpart.csv` is read instead, so the panel fills in as detection proceeds. Toggle the panel with the [activity icon](buzzdetect-toggle) in the **Neurons** section header, in the left sidebar — that section, and the panel it opens, appear whenever the project names a buzzdetect directory. Any column with numeric content becomes a neuron, not only ones named `activation_...` — that prefix is stripped from the label by default, but **Advanced** has a checkbox to keep it. A column that isn't numeric is left out rather than breaking the read, and a cell with no value (blank, `NA`, and a few other common spellings) is skipped rather than plotted — its neuron's line just passes over that frame.\n\nIf the directory holds a `buzzdetect_manifest.json` (buzzdetect writes one at the top of every results folder), the model's suggested thresholds from it fill in **Detection at** for any neuron you haven't set or cleared yourself, and the log names what it filled."; },
  ...
  transport: {
    ...
    get p_volume() { return getOverride('help.transport.p_volume') ?? "The [volume slider](volume-control) supports up to 8× gain boost (past center), for files recorded very quiet; scroll over it to nudge the level. An output limiter catches the loud end so a slip up the slider can't blow your ears. Press `M` to mute. Right-click it, or the play button, to access **Restart Audio**, which re-initialises the audio engine (useful after an audio device change on Windows). In a narrow window the slider folds behind the mute icon — hover the icon to bring it back."; },
  },
  ...
  tools: {
    ...
    get p_manage() { return getOverride('help.tools.p_manage') ?? "Open [Annotation Tool Settings](tool-palette) (gear icon) to manage tools: drag tools between hotkey slots and the Unassigned bin, or click a tool's gear to edit its label and color — the editor has a red **Delete tool** button (deletes are undoable). The **Unassign All** button (or `{mod}+Shift+B` from the main window) frees every tool's hotkey in one step, without deleting any tools. To free a single hotkey without deleting the tool, click the unassign icon on a keyed tool or middle-click its row; middle-click a tool in the Unassigned bin to give it the next free key. Type into an empty hotkey slot's **Add tool** field to assign an existing tool by name (pick it from the dropdown) or, if the name is new, open the editor to create a tool on that key; in the Unassigned bin, click **New tool** to create one directly. In the **Labels** palette itself, middle-click a hotkey chip or click the unassign icon on hover to free its slot (the tool is kept, just unassigned), and while any of the nine slots is open an **Add tool** field sits below the list — type to pick an existing tool from the dropdown (arrow keys + Enter, or click; assigned to the next free key) or, for a new name, open the editor to create one. Either way the tool is readied for use as soon as it lands on a key. Annotations are linked to a tool by their label, so creating or renaming a tool to match a label instantly adopts every annotation carrying it — across all tracks. Tool configuration is saved per project: each tool is a folder under the project's **.seenote/annotation-tools/** directory, which can also hold example audio clips for that label."; },
  ...
  bulk: {
    get p_rename() { return getOverride('help.bulk.p_rename') ?? "`{mod}+F`, or the **Find Label** ([magnifying glass](tool-palette)) icon next to the [gear](tool-palette), summons the **Find & Rename** panel on the right-hand edge of the window — searching and bulk-renaming annotations by their text directly, without needing a matching tool. It's a panel rather than a dialog so the matches stay on screen while you work through them; closing it with its **X** or `Esc` dismisses it for good, and only those two entry points bring it back. Drag its inner edge to widen it. Type a label to search; **Project** (the default) scans every file, streaming results in alphabetically by file as it runs, **Folder** narrows that to the files beneath the folder the file panel has been entered into (it's greyed out at the top level, where it would match Project), and **Track** searches only the open track's (unsaved) annotations — this also decides what a rename applies to. Whole-project results are held in memory once scanned, so refining the query or flipping the toggles re-filters instantly instead of re-reading the project."; },
    get h_matching() { return getOverride('help.bulk.h_matching') ?? "How the query matches"; },
    get p_matching() { return getOverride('help.bulk.p_matching') ?? "Three pills beside the search field control matching, and all three are remembered per project. `Aa` makes the search case-sensitive — without it, matching folds case. `*a*` matches the search text anywhere in a label (e.g. `mech_` matches `quiet_mech_auto`). `.*` reads the query as a regular expression (already unanchored, so `buzz\\d` matches `foo_buzz3_bar`), and wins over `*a*` when both are on. A loose query can hit several different labels at once, so each match chip carries its own label text — what matched, not what you typed."; },
    get h_find() { return getOverride('help.bulk.h_find') ?? "Finding a match"; },
    get p_find() { return getOverride('help.bulk.p_find') ?? "Every match is a chip in the results list, in its tool's color, labelled with its start time, and — in a whole-project search — grouped under the recording it belongs to, whose name stays pinned to the top of the list while you scroll through its matches. Click one to open that file, scroll the spectrogram to it, and select the annotation — with an active selection bound around it, ready to edit, resize, or delete. The chevrons at the top right step to the previous and next match — as do `Alt+↑` / `Alt+↓` from anywhere in the window while the panel is open, and `Shift+Enter` / `Enter` from the search field — wrapping round at the ends, and the readout beside them says where in the list you are. The list doesn't go anywhere as you walk it: the current match stays highlighted and scrolls into view, across track switches and through any edits you make along the way. If you've entered a folder in the [file panel](file-panel) and the match lies outside it, the file panel steps back out to the media root — otherwise `{mod}+↑` / `{mod}+↓` would have lost their place in a list the open track isn't in. It doesn't follow the match down into its own folder; narrowing the browser to a subtree is yours to choose."; },
    get h_rename() { return getOverride('help.bulk.h_rename') ?? "Renaming matches"; },
    get p_rename2() { return getOverride('help.bulk.p_rename2') ?? "**Rename matches**, at the foot of the panel, is always open: type a replacement label, then either **Rename Selected** — renames just the one match currently selected in the results list — or **Rename All**, which renames every annotation the search currently matches: current track only, the entered folder, or the whole project, per the scope chosen above."; },
  },
