...
export const help = {
  ...
  bulk: {
    get p_rename() { return getOverride('help.bulk.p_rename') ?? "`{mod}+F`, or the **Find Label** ([magnifying glass](tool-palette)) or **Mass Rename** ([text-cursor](tool-palette)) icons next to the [gear](tool-palette), all open the same **Find & Rename Label** dialog — searching and bulk-renaming annotations by their text directly, without needing a matching tool. Type a label to search; **Current track** searches only the open track's (unsaved) annotations, **Whole project** scans every file, streaming results in alphabetically by file as it runs — this also decides what a rename below applies to. Check **Partial** to match the search text anywhere in a label (e.g. `mech_` matches `quiet_mech_auto`), or **Regex** to search with a regular expression (already unanchored, so `buzz\\d` matches `foo_buzz3_bar`); with either on, results show each match's own label since it may differ from what you typed. Both toggles are remembered per project."; },
