...
export class AudioEngine implements PlaybackTransport {
  ...
  private async _prefetchLoop(myPlayId: number, continueFromCtx?: number): Promise<void> {
    ...
    while (this.playId === myPlayId && this.streamId === handle.stream_id) {
      ...
      // the lead time we add on top of it. Compensating only the overrun (as
      // this did) leaves the playhead permanently UNDERRUN_LEAD_SEC ahead of
      // the audio, and since nothing ever re-anchors mid-play, the error
      // accumulates: a dozen underruns is a quarter second of the sound
      // trailing the cursor, which is exactly the window annotations get placed
      // in.
      ...
      if (this.chunksScheduled === 1) {
        const elapsedMs = Math.round(performance.now() - this.playStartedAtMs);
        this._log(`first chunk scheduled mediaStart=${chunkMediaStart.toFixed(3)}s in=${inputFrames}f out=${outputFrames}f ch=${chunk.channels} (${elapsedMs}ms after play)`);
        if (this.slowDecodeTimer !== null) {
          clearInterval(this.slowDecodeTimer);
          this.slowDecodeTimer = null;
        }
